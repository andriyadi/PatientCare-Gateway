...
This display is an in memory buffer that flushes to the device.
      ...
      drawPixel: (x, y, color=@WHITE) ->
        if x < 0 || x >= @width || y < 0 || y >= @height then return
...
The basic print method, display text, replacing all text.
      ...
      startscrollright: (start, stop) ->
        @control @RIGHT_HORIZONTAL_SCROLL
        @control 0
        @control start
        @control 0
        @control stop
        @control 0
        @control 0xFF
        @control @ACTIVATE_SCROLL

      startscrollleft: (start, stop) ->
        @control @LEFT_HORIZONTAL_SCROLL
        @control 0
        @control start
        @control 0
        @control stop
