...
uint32_t loraChannelArray[MAX_NB_CHANNEL] = { CH_10_868, CH_11_868, CH_12_868,
		CH_13_868, CH_14_868, CH_15_868, CH_16_868, CH_17_868, CH_18_868 };
...
boolean sendDataToNodeRequested = false; //by Andri
uint8_t sendDataToNodeAddress = 0;
...
void startConfig() {
	...
	if (optBW != 0 || optCR != 0 || optSF != 0) {
		...
		e = sx1272.setBW(
				(optBW == 125) ? BW_125 : ((optBW == 250) ? BW_250 : BW_500));
...
//by Andri
	...
	if (sendDataToNodeRequested && sendDataToNodeAddress != 0) {

		sx1272.setPacketType(PKT_TYPE_DATA);
		PRINT_CSTSTR("%s", "Sending back data to device\n");

		printf("CMD: %s\n", cmd);
		//char sentData[13] = "Hello world!";
		e = sx1272.sendPacketTimeout(sendDataToNodeAddress, (uint8_t*) cmd, strlen(cmd), 2000);
		if (!e) {
			sendDataToNodeAddress = 0;
			sendDataToNodeRequested = false;
		}
	}
...
///END/////////////////////////////////////////////////
		...
		if (!e) {
			...
			sprintf(sprintf_buf,
					"--- rxlora. dst=%d type=0x%.2X src=%d seq=%d len=%d SNR=%d RSSIpkt=%d BW=%d CR=4/%d SF=%d\n",
					...
					(sx1272._bandwidth == BW_125) ?
							125 : ((sx1272._bandwidth == BW_250) ? 250 : 500),
...
//			PRINT_CSTSTR("%s", "Sending back data to device\n");
//			char sentData[13] = "Hello world!";
//			e = sx1272.sendPacketTimeout(6, (uint8_t*) sentData, strlen(sentData), 2000);
		}
	}
	...
	if (receivedFromSerial || receivedFromLoRa) {
		...
		if (cmd[i] == '/' && cmd[i + 1] == '@') {
			...
			if ((receivedFromLoRa && cmd[i] != 'U' && !unlocked)
					|| !unlocked_try) {
...
///END/////////////////////////////////////////////////
			...
			case 'P':
				...
				if (cmd[i + 1] == 'L' || cmd[i + 1] == 'H' || cmd[i + 1] == 'M'
						|| cmd[i + 1] == 'x' || cmd[i + 1] == 'X') {
					loraPower = cmd[i + 1];
					...
					PRINT_CSTSTR("%s",
							"Invalid Power. L, H, M, x or X accepted.\n");
				break;
			...
			case 'O':
				...
				if (cmd[i + 1] == 'N') {
					...
					PRINT_CSTSTR("%s",
							"Invalid command. ON or OFF accepted.\n");
				break;
...
//by Andri
#define INPUT_FIFO_NAME "/tmp/lora_fifo"
...
void create_fifo() {
	...
	unistd::unlink(INPUT_FIFO_NAME);

	printf("Making FIFO...\n");
	result = mkfifo(INPUT_FIFO_NAME, 0666);		//(This will fail if the fifo already exists in the system from the app previously running, this is fine)
	if (result == 0)
	{
		//FIFO CREATED
		printf("New FIFO created: %s\n", INPUT_FIFO_NAME);

		if (unistd::chown(INPUT_FIFO_NAME, 1000, 0) != 0) {

		}
		else {
			struct stat info;
	      	stat(INPUT_FIFO_NAME, &info);
	      	printf("after chown(), owner is %d and group is %d\n",
	             info.st_uid, info.st_gid);
	    }
	}

	printf("Process %d opening FIFO %s\n", unistd::getpid(), INPUT_FIFO_NAME);
	input_fifo_filestream = open(INPUT_FIFO_NAME, (O_RDONLY | O_NONBLOCK));
	//Possible flags:
...
void check_fifo() {
	...
	if (input_fifo_filestream != -1)
	{
		...
		int rx_length = unistd::read(input_fifo_filestream, (void*)rx_buffer, 255);		//Filestream, buffer to store in, number of bytes to read (max)
		if (rx_length < 0)
		{
		...
		else
		{
			...
			sprintf(sprintf_buf, "FIFO %i bytes read : %s\n", rx_length, rx_buffer);
			PRINT_STR("%s", sprintf_buf);

			if (rx_buffer[0] == '/') {

				char *payloadPart = strtok((char *)rx_buffer, "/");
				sendDataToNodeAddress = atoi(payloadPart);

				payloadPart = strtok(NULL, "/");

				sendDataToNodeRequested = true;
				//strcpy(cmd, (const char *)rx_buffer);
				strcpy(cmd, payloadPart);
			}
		}
	}
}
...
//		PRINT_CSTSTR("%s", "...\n");
	...
	//by Andri
	(void)unistd::close(input_fifo_filestream);
